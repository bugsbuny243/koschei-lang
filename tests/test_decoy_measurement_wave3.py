...
class DecoyMeasurementWave3Tests(unittest.TestCase):
    def stale_grant(self):
        return issue_read_grant(
            ...
            nonce="wave3-stale-nonce-0001",
            authorization_key=AUTH_KEY,
        )

    def test_ten_thousand_unauthorized_probes_never_read_canonical(self):
        metrics = measure_unauthorized_probes(
            project_id="p-01",
            object_id=OID,
            start_epoch=100,
            probes=10_000,
            stale_grant=self.stale_grant(),
            deception_key=DECEPTION_KEY,
            authorization_key=AUTH_KEY,
            canonical_reader=lambda _: SECRET,
        )
    ...
    def test_observable_size_band_is_bounded_for_current_generator(self):
        ...
        # Regression guard only: prevents accidental epoch-dependent payload blow-up.
