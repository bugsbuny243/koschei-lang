...
def env(builder,suffix,keybyte,root):
    ebytes=("env-"+suffix).encode(); wbytes=("workload-"+builder).encode(); raw=("quote-"+suffix).encode(); verifier_artifact=("attestation-verifier-"+suffix).encode(); remote_key=keybyte.upper()*32; env_key=keybyte*32
    ...
    remote=verify_remote_attestation_with_abi_v1(abi=abi,verifier_artifact_bytes=verifier_artifact,raw_evidence_bytes=raw,current_epoch=71,verifier=lambda _:RemoteAttestationVerificationResultV1(root,ebytes,wbytes,71,80),remote_attestation_verifier_key=remote_key)
    ...
    return receipt,ebytes,wbytes,env_key,remote,raw,remote_key,abi,verifier_artifact
def chain():
    ...
    env_a,env_a_bytes,work_a,env_a_key,remote_a,raw_a,remote_a_key,attabi_a,attart_a=env("builder-a","a",b"m","root-a")
    env_b,env_b_bytes,work_b,env_b_key,remote_b,raw_b,remote_b_key,attabi_b,attart_b=env("builder-b","b",b"n","root-b")
