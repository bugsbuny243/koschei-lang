...
def env(builder,suffix,keybyte):
    ebytes=("env-"+suffix).encode(); wbytes=("workload-"+builder).encode(); key=keybyte*32
    return attest_builder_environment_v1(builder_id=builder,environment_id="environment-"+suffix,environment_bytes=ebytes,workload_bytes=wbytes,attestation_authority_id="koschei-build-attestor",epoch=71,environment_attestation_key=key),ebytes,wbytes,key
def chain():
    ...
    env_a,env_a_bytes,work_a,env_a_key=env("builder-a","a",b"m"); env_b,env_b_bytes,work_b,env_b_key=env("builder-b","b",b"n")
    ...
    base,gated=admit_reproducible_verifier_artifact_v1(reproducible_admission_key=gak,runtime_admission_key=rk,build_provenance_key=bk,reproducibility_key=rkey,builder_a_key=akey,builder_b_key=bkey,reproducibility_receipt=repro,builder_a=obs_a,builder_b=obs_b,builder_a_toolchain=tc_a,builder_b_toolchain=tc_b,builder_a_toolchain_artifact_bytes=tc_a_bytes,builder_b_toolchain_artifact_bytes=tc_b_bytes,builder_a_toolchain_signing_key=tc_a_key,builder_b_toolchain_signing_key=tc_b_key,builder_a_environment=env_a,builder_b_environment=env_b,builder_a_environment_bytes=env_a_bytes,builder_b_environment_bytes=env_b_bytes,builder_a_workload_bytes=work_a,builder_b_workload_bytes=work_b,builder_a_environment_attestation_key=env_a_key,builder_b_environment_attestation_key=env_b_key,build_toolchain=tc_a,build_toolchain_artifact_bytes=tc_a_bytes,build_toolchain_signing_key=tc_a_key,verified_input=verified,mir=mir,proof=proof,provenance=provenance,artifact_bytes=artifact,adapter_abi=abi)
    return locals()
def test_two_builders_require_distinct_attested_environments():
    x=chain(); assert x['repro'].builder_a_environment_attestation_digest==x['env_a'].attestation_digest; assert x['repro'].builder_b_environment_attestation_digest==x['env_b'].attestation_digest
def test_same_environment_identity_cannot_count_twice():
    x=chain(); forged_env_b=replace(x['env_b'],environment_id=x['env_a'].environment_id)
    with pytest.raises(VerifierReproducibleBuildV1Error,match="distinct attested environments"):
        seal_reproducible_build_receipt_v1(reproducibility_key=x['rkey'],builder_a_key=x['akey'],builder_b_key=x['bkey'],builder_a=x['obs_a'],builder_b=x['obs_b'],builder_a_toolchain=x['tc_a'],builder_b_toolchain=x['tc_b'],builder_a_toolchain_artifact_bytes=x['tc_a_bytes'],builder_b_toolchain_artifact_bytes=x['tc_b_bytes'],builder_a_toolchain_signing_key=x['tc_a_key'],builder_b_toolchain_signing_key=x['tc_b_key'],builder_a_environment=x['env_a'],builder_b_environment=forged_env_b,builder_a_environment_bytes=x['env_a_bytes'],builder_b_environment_bytes=x['env_b_bytes'],builder_a_workload_bytes=x['work_a'],builder_b_workload_bytes=x['work_b'],builder_a_environment_attestation_key=x['env_a_key'],builder_b_environment_attestation_key=x['env_b_key'],verified_input=x['verified'],mir=x['mir'],proof=x['proof'],artifact_bytes=x['artifact'])
def test_same_environment_measurement_cannot_count_twice():
    x=chain(); shared_bytes=x['env_a_bytes']; env_b=attest_builder_environment_v1(builder_id="builder-b",environment_id="environment-b2",environment_bytes=shared_bytes,workload_bytes=x['work_b'],attestation_authority_id="koschei-build-attestor",epoch=71,environment_attestation_key=x['env_b_key'])
    with pytest.raises(VerifierReproducibleBuildV1Error,match="distinct attested environments"):
        seal_reproducible_build_receipt_v1(reproducibility_key=x['rkey'],builder_a_key=x['akey'],builder_b_key=x['bkey'],builder_a=x['obs_a'],builder_b=x['obs_b'],builder_a_toolchain=x['tc_a'],builder_b_toolchain=x['tc_b'],builder_a_toolchain_artifact_bytes=x['tc_a_bytes'],builder_b_toolchain_artifact_bytes=x['tc_b_bytes'],builder_a_toolchain_signing_key=x['tc_a_key'],builder_b_toolchain_signing_key=x['tc_b_key'],builder_a_environment=x['env_a'],builder_b_environment=env_b,builder_a_environment_bytes=x['env_a_bytes'],builder_b_environment_bytes=shared_bytes,builder_a_workload_bytes=x['work_a'],builder_b_workload_bytes=x['work_b'],builder_a_environment_attestation_key=x['env_a_key'],builder_b_environment_attestation_key=x['env_b_key'],verified_input=x['verified'],mir=x['mir'],proof=x['proof'],artifact_bytes=x['artifact'])
def test_environment_attestation_tamper_rejects_builder_observation():
    x=chain(); forged=replace(x['env_a'],workload_digest='0'*64)
    with pytest.raises(ValueError): x['obs_a'].assert_authenticated(builder_key=x['akey'],verified_input=x['verified'],mir=x['mir'],proof=x['proof'],toolchain=x['tc_a'],toolchain_artifact_bytes=x['tc_a_bytes'],toolchain_signing_key=x['tc_a_key'],environment=forged,environment_bytes=x['env_a_bytes'],workload_bytes=x['work_a'],environment_attestation_key=x['env_a_key'],artifact_bytes=x['artifact'])
