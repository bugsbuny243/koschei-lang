...
def env(builder,suffix,keybyte,root):
    ebytes=("env-"+suffix).encode(); wbytes=("workload-"+builder).encode(); raw=("quote-"+suffix).encode(); verifier_artifact=("attestation-verifier-"+suffix).encode(); remote_key=keybyte.upper()*32; env_key=keybyte*32; root_key=(suffix.encode()*32)[:32]; admission_key=(suffix.upper().encode()*32)[:32]; generation_state=TrustAnchorGenerationStateV1()
    ...
    manifest=seal_trust_anchor_manifest_v1(anchor_id="offline-root-"+suffix,generation=1,abi=abi,allowed_trust_root_ids=(root,),valid_from_epoch=70,expires_before_epoch=90,root_signing_key=root_key)
    root_admission=admit_attestation_verifier_from_trust_anchor_v1(manifest=manifest,abi=abi,verifier_artifact_bytes=verifier_artifact,current_epoch=71,root_signing_key=root_key,runtime_admission_key=admission_key,generation_state=generation_state)
...
def chain():
    ...
    return locals()
def test_two_builders_bind_current_trust_generations():
    x=chain(); assert x['repro'].builder_a_trust_anchor_generation==1; assert x['repro'].builder_b_trust_anchor_generation==1; assert x['repro'].builder_a_trust_anchor_manifest_digest==x['manifest_a'].manifest_digest
def test_newer_generation_invalidates_old_builder_and_repro_receipt():
    x=chain(); newer=seal_trust_anchor_manifest_v1(anchor_id=x['manifest_a'].anchor_id,generation=2,abi=x['attabi_a'],allowed_trust_root_ids=("root-a",),valid_from_epoch=70,expires_before_epoch=90,root_signing_key=x['root_key_a']); x['generation_state_a'].observe(newer,root_signing_key=x['root_key_a'],abi=x['attabi_a'],current_epoch=71)
    with pytest.raises(TrustAnchorAdmissionV1Error,match="current observed generation"):
        x['env_a'].assert_authenticated(environment_attestation_key=x['env_a_key'],environment_bytes=x['env_a_bytes'],workload_bytes=x['work_a'],remote_evidence=x['remote_a'],raw_remote_evidence_bytes=x['raw_a'],remote_attestation_verifier_key=x['remote_a_key'],current_epoch=71,trust_anchor_generation_state=x['generation_state_a'])
    with pytest.raises(TrustAnchorAdmissionV1Error,match="current observed generation"):
        x['repro'].assert_authenticated(**x['common'],require_distinct_trust_roots=True)
def test_same_trust_root_rejected_when_policy_requires_distinct_roots():
    ...
    with pytest.raises(ValueError): x['env_b'].assert_authenticated(environment_attestation_key=x['env_b_key'],environment_bytes=x['env_b_bytes'],workload_bytes=x['work_b'],remote_evidence=same_root,raw_remote_evidence_bytes=x['raw_b'],remote_attestation_verifier_key=x['remote_b_key'],current_epoch=71,trust_anchor_generation_state=x['generation_state_b'])
def test_environment_attestation_tamper_rejects_builder_observation():
    x=chain(); forged=replace(x['env_a'],workload_digest='0'*64)
    with pytest.raises(ValueError): x['obs_a'].assert_authenticated(builder_key=x['akey'],verified_input=x['verified'],mir=x['mir'],proof=x['proof'],toolchain=x['tc_a'],toolchain_artifact_bytes=x['tc_a_bytes'],toolchain_signing_key=x['tc_a_key'],environment=forged,environment_bytes=x['env_a_bytes'],workload_bytes=x['work_a'],environment_attestation_key=x['env_a_key'],remote_evidence=x['remote_a'],raw_remote_evidence_bytes=x['raw_a'],remote_attestation_verifier_key=x['remote_a_key'],current_epoch=71,trust_anchor_generation_state=x['generation_state_a'],artifact_bytes=x['artifact'])
