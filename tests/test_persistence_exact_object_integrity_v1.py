...
class PersistenceExactObjectIntegrityV1Tests(unittest.TestCase):
    ...
    def test_load_open_is_nonblocking_even_if_shape_check_is_raced_to_fifo(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            # Simulate the narrow race: a prior shape check saw a regular file,
            # but the object became a FIFO before the real data open. Defense in
            # depth must not block waiting for a writer; O_NONBLOCK + post-open
            # fstat must reject it immediately.
            with patch(
