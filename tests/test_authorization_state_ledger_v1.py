...
class AuthorizationStateLedgerV1Tests(unittest.TestCase):
    def test_current_head_can_create_fresh_execution_snapshot(self):
        state = initial_state()
    ...
    def test_revocation_advances_head_and_rejects_old_active_state_rollback(self):
        ...
        with self.assertRaisesRegex(AuthorizationTransitionV1Error, "current monotonic head"):
    ...
    def test_transition_commit_must_extend_current_head(self):
        active = initial_state()
        ledger = AuthorizationStateLedgerV1()
        ledger.register_initial(active)
    ...
    def test_unregistered_state_fails_closed(self):
        with self.assertRaisesRegex(AuthorizationTransitionV1Error, "head is unavailable"):
