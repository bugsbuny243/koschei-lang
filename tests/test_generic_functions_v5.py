...
"""
    ...
    def test_duplicate_type_parameter_is_rejected(self) -> None:
        program = parse("fn identity<T, T>(value: T) -> T { return value }")
        with self.assertRaises(SemanticError) as raised:
            check_typed_hir(program)
        self.assertEqual(raised.exception.code, "KS1307")
    ...
    def test_runtime_generic_conflict_is_not_mislabeled_as_capability_attack(self) -> None:
        from koschei.interpreter import Interpreter, KoscheiRuntimeError

        program = parse("fn same<T>(left: T, right: T) -> T { return left }")
        interpreter = Interpreter(program, [])
        with self.assertRaises(KoscheiRuntimeError) as raised:
            interpreter._call_function(program.declarations[0], [1, "two"])
