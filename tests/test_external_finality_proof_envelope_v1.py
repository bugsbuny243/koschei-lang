...
def chain(*, finality_state="finalized"):
    ...
    txid = b"pi-transaction-reference:71"
    consumption, effect_receipt, effect_result = execute_effect_with_receipt_v1(
        ...
        request=request, current_epoch=71, effect=lambda _: txid,
    )
    ...
    native_receipt = verify_pi_native_payment_response_v1(
        effect_envelope=effect_envelope,
        effect_receipt=effect_receipt,
        effect_result_txid_bytes=effect_result,
        raw_pi_response_bytes=raw_pi_response,
        observed_epoch=72,
        verifier=lambda raw: ProviderNativeVerificationResultV1(
            reference_bytes=txid,
            proof_bytes=b"canonical-pi-provider-proof:71:" + raw,
            state=finality_state,
        ),
        provider_native_verifier_key=nk,
    )
    ...
    finality = seal_external_finality_proof_envelope_v1(
        ...
        effect_result_bytes=effect_result, raw_provider_response_bytes=raw_pi_response,
        native_receipt=native_receipt, base=base, grant=grant, evidence=evidence,
        ...
        effect_key=ek, provider_native_verifier_key=nk,
        provider_verifier_key=vk, finality_key=fk,
    )
    return locals()


def validate(x, envelope=None, raw_response=None):
    (envelope or x["finality"]).assert_valid(
        effect_envelope=x["effect_envelope"], effect_receipt=x["effect_receipt"],
        effect_result_bytes=x["effect_result"],
        raw_provider_response_bytes=raw_response or x["raw_pi_response"],
        native_receipt=x["native_receipt"], base=x["base"], grant=x["grant"],
        evidence=x["evidence"], mir=x["mir"], request=x["request"], proof=x["proof"],
        bound=x["bound"], basis=x["basis"], decision=x["decision"], permit=x["permit"],
        consumption=x["consumption"], verdict=x["verdict"], attestation=x["attestation"],
        decision_key=x["dk"], runtime_key=x["rk"], effect_key=x["ek"],
        provider_native_verifier_key=x["nk"], provider_verifier_key=x["vk"], finality_key=x["fk"],
    )


def test_provider_finalized_requires_raw_response_native_verification_and_full_chain():
    x = chain()
    validate(x)
    ...
    assert x["finality"].provider_native_verification_receipt_digest == x["native_receipt"].receipt_digest
    assert x["finality"].raw_provider_response_digest == x["native_receipt"].raw_response_digest
...
def test_finality_envelope_relabel_or_reference_rebinding_is_rejected():
    ...
    for forged in (
        ...
        replace(x["finality"], raw_provider_response_digest=h("other-response")),
    ):
        with pytest.raises(ExternalFinalityProofEnvelopeV1Error):
            validate(x, forged)


def test_full_finality_proof_rejects_different_raw_provider_response():
    x = chain()
    with pytest.raises(ValueError, match="raw response mismatch"):
        validate(x, raw_response=b"different-pi-response")
