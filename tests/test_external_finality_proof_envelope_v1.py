...
def tc(version,byte):
    artifact=("koschei-compiler-"+version).encode(); key=byte*32; return attest_toolchain_provenance_v1(toolchain_id="koschei-compiler",toolchain_version=version,toolchain_artifact_bytes=artifact,build_profile="release",toolchain_signing_key=key),artifact,key
def env(builder,suffix,keybyte,root):
    eb=("env-"+suffix).encode(); wb=("workload-"+builder).encode(); raw=("quote-"+suffix).encode(); verifier_artifact=("attestation-verifier-"+suffix).encode(); rkey=keybyte.upper()*32; ekey=keybyte*32
    ...
    remote=verify_remote_attestation_with_abi_v1(abi=attabi,verifier_artifact_bytes=verifier_artifact,raw_evidence_bytes=raw,current_epoch=71,verifier=lambda _:RemoteAttestationVerificationResultV1(root,eb,wb,71,80),remote_attestation_verifier_key=rkey)
    receipt=attest_builder_environment_v1(builder_id=builder,environment_id="environment-"+suffix,environment_bytes=eb,workload_bytes=wb,attestation_authority_id="build-attestor",environment_attestation_key=ekey,remote_evidence=remote,raw_remote_evidence_bytes=raw,remote_attestation_verifier_key=rkey)
    return receipt,eb,wb,ekey,remote,raw,rkey,attabi,verifier_artifact
def chain(finality_state="finalized"):
    mir,proof=proof_for(SOURCE); request=seal_effect_request(mir,effect_id="pi-finality-71",subject="withdrawal",operation="subscription.enable",request_digest=h("payload-71"),identity_digest=h("pi-user-71"),epoch=71,nonce_digest=h("nonce-71")); bound=bind_proof_to_request(mir,request,proof); basis=derive_canonical_authority_basis_v1(mir=mir,request=request,proof=proof,bound=bound)
    ...
    verifier_mir,verifier_proof=proof_for(VERIFIER_SOURCE); verified_input=derive_verified_ir_build_input_v1(mir=verifier_mir,proof=verifier_proof); verifier_artifact=b"compiled-pi-finality-verifier-v1"; tc_a,tc_a_bytes,tc_a_key=tc("a",b"x"); tc_b,tc_b_bytes,tc_b_key=tc("b",b"y")
    env_a,env_a_bytes,work_a,env_a_key,remote_a,raw_a,remote_a_key,attabi_a,attart_a=env("builder-a","a",b"m","root-a"); env_b,env_b_bytes,work_b,env_b_key,remote_b,raw_b,remote_b_key,attabi_b,attart_b=env("builder-b","b",b"n","root-b")
    provenance=attest_verifier_build_from_verified_ir_v1(verified_input=verified_input,mir=verifier_mir,proof=verifier_proof,artifact_bytes=verifier_artifact,toolchain=tc_a,toolchain_artifact_bytes=tc_a_bytes,toolchain_signing_key=tc_a_key,build_profile="release-reproducible",build_provenance_key=bk); adapter_abi=seal_provider_adapter_abi_v1(provider_id="pi",adapter_id="pi-payment-finality",schema_id="opaque-pi-payment-response",schema_version="v1",verifier_implementation_digest=measure_verifier_artifact_v1(verifier_artifact))
    obs_a=attest_builder_observation_v1(builder_id="builder-a",builder_key=ba,verified_input=verified_input,mir=verifier_mir,proof=verifier_proof,toolchain=tc_a,toolchain_artifact_bytes=tc_a_bytes,toolchain_signing_key=tc_a_key,environment=env_a,environment_bytes=env_a_bytes,workload_bytes=work_a,environment_attestation_key=env_a_key,remote_evidence=remote_a,raw_remote_evidence_bytes=raw_a,remote_attestation_verifier_key=remote_a_key,current_epoch=71,artifact_bytes=verifier_artifact,build_profile="release-reproducible")
    obs_b=attest_builder_observation_v1(builder_id="builder-b",builder_key=bb,verified_input=verified_input,mir=verifier_mir,proof=verifier_proof,toolchain=tc_b,toolchain_artifact_bytes=tc_b_bytes,toolchain_signing_key=tc_b_key,environment=env_b,environment_bytes=env_b_bytes,workload_bytes=work_b,environment_attestation_key=env_b_key,remote_evidence=remote_b,raw_remote_evidence_bytes=raw_b,remote_attestation_verifier_key=remote_b_key,current_epoch=71,artifact_bytes=verifier_artifact,build_profile="release-reproducible")
    ...
    repro=seal_reproducible_build_receipt_v1(reproducibility_key=repro_key,builder_a_key=ba,builder_b_key=bb,verified_input=verified_input,mir=verifier_mir,proof=verifier_proof,artifact_bytes=verifier_artifact,**common)
    runtime_admission,reproducible_admission=admit_reproducible_verifier_artifact_v1(reproducible_admission_key=gate_key,runtime_admission_key=ak,build_provenance_key=bk,reproducibility_key=repro_key,builder_a_key=ba,builder_b_key=bb,reproducibility_receipt=repro,build_toolchain=tc_a,build_toolchain_artifact_bytes=tc_a_bytes,build_toolchain_signing_key=tc_a_key,verified_input=verified_input,mir=verifier_mir,proof=verifier_proof,provenance=provenance,artifact_bytes=verifier_artifact,adapter_abi=adapter_abi,**common)
...
def test_provider_finalized_requires_abi_bound_remote_attested_builder_environments():
    x=chain(); validate(x); x['finality'].assert_finalized(); assert x['remote_a'].attestation_verifier_abi_digest==x['attabi_a'].abi_digest; assert x['remote_b'].attestation_verifier_abi_digest==x['attabi_b'].abi_digest
