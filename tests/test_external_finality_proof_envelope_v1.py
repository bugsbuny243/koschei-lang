...
def env(builder,suffix,keybyte):
    eb=("env-"+suffix).encode(); wb=("workload-"+builder).encode(); key=keybyte*32; return attest_builder_environment_v1(builder_id=builder,environment_id="environment-"+suffix,environment_bytes=eb,workload_bytes=wb,attestation_authority_id="build-attestor",epoch=71,environment_attestation_key=key),eb,wb,key
def chain(finality_state="finalized"):
    ...
    verifier_mir,verifier_proof=proof_for(VERIFIER_SOURCE); verified_input=derive_verified_ir_build_input_v1(mir=verifier_mir,proof=verifier_proof); verifier_artifact=b"compiled-pi-finality-verifier-v1"; tc_a,tc_a_bytes,tc_a_key=tc("a",b"x"); tc_b,tc_b_bytes,tc_b_key=tc("b",b"y"); env_a,env_a_bytes,work_a,env_a_key=env("builder-a","a",b"m"); env_b,env_b_bytes,work_b,env_b_key=env("builder-b","b",b"n")
    ...
    obs_a=attest_builder_observation_v1(builder_id="builder-a",builder_key=ba,verified_input=verified_input,mir=verifier_mir,proof=verifier_proof,toolchain=tc_a,toolchain_artifact_bytes=tc_a_bytes,toolchain_signing_key=tc_a_key,environment=env_a,environment_bytes=env_a_bytes,workload_bytes=work_a,environment_attestation_key=env_a_key,artifact_bytes=verifier_artifact,build_profile="release-reproducible"); obs_b=attest_builder_observation_v1(builder_id="builder-b",builder_key=bb,verified_input=verified_input,mir=verifier_mir,proof=verifier_proof,toolchain=tc_b,toolchain_artifact_bytes=tc_b_bytes,toolchain_signing_key=tc_b_key,environment=env_b,environment_bytes=env_b_bytes,workload_bytes=work_b,environment_attestation_key=env_b_key,artifact_bytes=verifier_artifact,build_profile="release-reproducible")
...
def test_provider_finalized_requires_distinct_attested_builder_environments():
    x=chain(); validate(x); x['finality'].assert_finalized(); assert x['env_a'].environment_id!=x['env_b'].environment_id; assert x['repro'].builder_a_environment_attestation_digest==x['env_a'].attestation_digest; assert x['repro'].builder_b_environment_attestation_digest==x['env_b'].attestation_digest
