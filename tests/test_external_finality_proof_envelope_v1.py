...
def env(builder,suffix,keybyte,root):
    eb=("env-"+suffix).encode(); wb=("workload-"+builder).encode(); raw=("quote-"+suffix).encode(); rkey=keybyte.upper()*32; ekey=keybyte*32
    remote=attest_remote_attestation_evidence_v1(attestation_provider_id="bootstrap-test-attestor",trust_root_id=root,raw_evidence_bytes=raw,environment_bytes=eb,workload_bytes=wb,observed_epoch=71,expires_before_epoch=80,remote_attestation_verifier_key=rkey)
    receipt=attest_builder_environment_v1(builder_id=builder,environment_id="environment-"+suffix,environment_bytes=eb,workload_bytes=wb,attestation_authority_id="build-attestor",epoch=71,environment_attestation_key=ekey,remote_evidence=remote,raw_remote_evidence_bytes=raw,remote_attestation_verifier_key=rkey,current_epoch=71)
    return receipt,eb,wb,ekey,remote,raw,rkey
...
def test_provider_finalized_requires_remote_attested_builder_environments():
    x=chain(); validate(x); x['finality'].assert_finalized(); assert x['remote_a'].trust_root_id!=x['remote_b'].trust_root_id; assert x['repro'].builder_a_remote_attestation_evidence_digest==x['remote_a'].evidence_digest; assert x['repro'].builder_b_remote_attestation_evidence_digest==x['remote_b'].evidence_digest
