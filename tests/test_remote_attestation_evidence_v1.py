...
def test_remote_evidence_binds_root_generation_admission_abi_artifact_and_freshness():
    ...
    assert x['remote'].trust_anchor_id==x['manifest'].anchor_id
    assert x['remote'].trust_anchor_generation==1
...
def test_newer_generation_blocks_older_manifest_before_callback():
    x=fixture()
    newer=seal_trust_anchor_manifest_v1(anchor_id="offline-root-v1",generation=2,abi=x['abi'],allowed_trust_root_ids=("root-a",),valid_from_epoch=9,expires_before_epoch=20,root_signing_key=x['root_key'])
    ...
    with pytest.raises(TrustAnchorAdmissionV1Error,match="current observed generation"):
        verify_remote_attestation_with_abi_v1(**k,verifier=lambda _:calls.append(1))
    assert calls==[]
    assert x['generation_state'].highest_generation("offline-root-v1")==2

def test_same_generation_different_manifest_is_equivocation():
    x=fixture()
    conflicting=seal_trust_anchor_manifest_v1(anchor_id="offline-root-v1",generation=1,abi=x['abi'],allowed_trust_root_ids=("root-a","root-b"),valid_from_epoch=9,expires_before_epoch=20,root_signing_key=x['root_key'])
    with pytest.raises(TrustAnchorAdmissionV1Error,match="equivocation"):
        admit_attestation_verifier_from_trust_anchor_v1(manifest=conflicting,abi=x['abi'],verifier_artifact_bytes=x['verifier_artifact'],current_epoch=10,root_signing_key=x['root_key'],runtime_admission_key=x['admission_key'],generation_state=x['generation_state'])
...
def test_revoked_verifier_manifest_cannot_admit_runtime():
    ...
    with pytest.raises(TrustAnchorAdmissionV1Error,match="revoked"):
        admit_attestation_verifier_from_trust_anchor_v1(manifest=revoked,abi=x['abi'],verifier_artifact_bytes=x['verifier_artifact'],current_epoch=10,root_signing_key=x['root_key'],runtime_admission_key=x['admission_key'],generation_state=state)

def test_expired_root_manifest_rejects_before_callback():
    x=fixture(); state=TrustAnchorGenerationStateV1(); expired=seal_trust_anchor_manifest_v1(anchor_id="offline-root-v1",generation=2,abi=x['abi'],allowed_trust_root_ids=("root-a",),valid_from_epoch=1,expires_before_epoch=10,root_signing_key=x['root_key']); calls=[]
    stale=replace(x['root_admission'],manifest_digest=expired.manifest_digest,manifest_generation=expired.generation)
