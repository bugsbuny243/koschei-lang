...
class CliUtf8StdioTests(unittest.TestCase):
    def test_ascii_public_streams_are_hardened_before_unicode_output(self) -> None:
        ...
        with patch("koschei.cli_entry.sys.stdout", stdout), patch("koschei.cli_entry.sys.stderr", stderr):
    ...
    def test_non_reconfigurable_streams_are_left_alone(self) -> None:
        class PlainStream:
            pass
        with patch("koschei.cli_entry.sys.stdout", PlainStream()), patch("koschei.cli_entry.sys.stderr", PlainStream()):
            _configure_utf8_stdio()
