...
def admitted(require_distinct_trust_roots=False):
...
def provider_kwargs(x,txid,receipt,envelope):
    return dict(provider_id="pi",adapter_abi=x['abi'],runtime_admission=x['base'],reproducible_admission=x['gate'],reproducibility_receipt=x['repro'],reproducibility_key=x['rkey'],builder_a_generation_state=x['generation_state_a'],builder_b_generation_state=x['generation_state_b'],provenance=x['provenance'],verifier_artifact_bytes=x['artifact'],build_provenance_key=x['bk'],runtime_admission_key=x['rak'],reproducible_admission_key=x['gate_key'],effect_envelope=envelope,effect_receipt=receipt,effect_result_bytes=txid,raw_response_bytes=b"raw",current_epoch=71,provider_native_verifier_key=b"n"*32)
...
def test_newer_builder_generation_rejects_old_provider_gate_before_callback():
    ...
    assert calls==[]
def test_expired_builder_trust_or_attestation_window_rejects_before_provider_callback():
    txid,receipt,envelope=effect_chain(); x=admitted(); calls=[]; k=provider_kwargs(x,txid,receipt,envelope); k['current_epoch']=80
    with pytest.raises(ValueError,match="not live at current epoch"): verify_provider_native_response_v1(**k,verifier=lambda raw:calls.append(1) or verifier_for(txid)(raw))
    assert calls==[]
def test_downstream_distinct_root_policy_rejects_weaker_gate_before_callback():
    txid,receipt,envelope=effect_chain(); x=admitted(False); calls=[]; k=provider_kwargs(x,txid,receipt,envelope); k['required_distinct_trust_roots']=True
    with pytest.raises(ValueError,match="requires distinct attestation trust roots"): verify_provider_native_response_v1(**k,verifier=lambda raw:calls.append(1) or verifier_for(txid)(raw))
    assert calls==[]
def test_distinct_root_policy_is_bound_into_receipt_and_admission():
    txid,receipt,envelope=effect_chain(); x=admitted(True); k=provider_kwargs(x,txid,receipt,envelope); k['required_distinct_trust_roots']=True
    native=verify_provider_native_response_v1(**k,verifier=verifier_for(txid)); assert native.provider_id=="pi"; assert x['repro'].require_distinct_trust_roots is True; assert x['gate'].require_distinct_trust_roots is True; assert x['gate'].builder_a_trust_root_id!=x['gate'].builder_b_trust_root_id
def test_pi_profile_cannot_bypass_current_reproducible_admission():
    txid,receipt,envelope=effect_chain(); x=admitted(); native=verify_pi_native_payment_response_v1(adapter_abi=x['abi'],runtime_admission=x['base'],reproducible_admission=x['gate'],reproducibility_receipt=x['repro'],reproducibility_key=x['rkey'],builder_a_generation_state=x['generation_state_a'],builder_b_generation_state=x['generation_state_b'],provenance=x['provenance'],verifier_artifact_bytes=x['artifact'],build_provenance_key=x['bk'],runtime_admission_key=x['rak'],reproducible_admission_key=x['gate_key'],effect_envelope=envelope,effect_receipt=receipt,effect_result_txid_bytes=txid,raw_pi_response_bytes=b"raw",current_epoch=71,verifier=verifier_for(txid),provider_native_verifier_key=b"n"*32); assert native.provider_id=="pi"
