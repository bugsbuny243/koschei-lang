...
v0.8, capability runtime ABI, cebirsel tip çekirdeği, immutable koleksiyonları, struct ve modülleri üretilen binary'ye taşır:
...
- Kullanıcı enum constructor'ları, nested exhaustive `match`, `Some/None`,
...
- Struct literal, alan erişimi ve nominal/yapısal eşitlik native runtime'da bulunur;
  capability taşıyan alanlar ikinci savunma katmanında KS3401 ile reddedilir.
- Import grafiği dependency-order ile tek binary'ye flatten edilir. Her modülün yerel
  fonksiyon ad alanı benzersizleştirilir; nested import alias'ları hem interpreter
  hem native runtime'da çağrılan modülün kendi bağlamında çözülür.

Native disk ABI daha zayıf bir yol kontrolüne GERİ DÜŞMEZ. Güvenli primitive'ler
bulunmayan hedefte derleme **KS4001** ile durur. Native backend'in gelecekte
karşılaşacağı desteklenmeyen bir yapı **KS4002** ile açıkça reddedilir; sessiz
yanlış çeviri yapılmaz.
