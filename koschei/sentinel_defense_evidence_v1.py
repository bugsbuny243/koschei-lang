...
class SentinelDefenseEvidenceResultV1:
    ...
    def __repr__(self) -> str:
        return (
            ...
            f"enforced={self.enforced}, evidence_head=<committed>)"
        )
...
def _evidence_command(action: str) -> str:
    # Ledger v1 has first-class quarantine/revoke. Other defensive acts are
    # recorded as observe until the ledger schema gains dedicated commands.
...
def execute_sentinel_defense_with_evidence_v1(
    ...
    *,
    ...
    The project id is accepted only to produce the ledger's correlation-safe
    commitment. It does not authorize the request. The request's project
    commitment is checked against the same project id before enforcement so
    evidence cannot be attached to an unrelated reality.
    """
    ...
    try:
    ...
    except SentinelDefenseAuthorityError as error:
        # Failed enforcement is still evidence-worthy, but authority internals
        # remain redacted by the lower boundary.
        failed = append_reality_evidence_v1(
            ledger,
        ...
        raise SentinelDefenseEvidenceError("defense enforcement failed; evidence appended") from error

    updated = append_reality_evidence_v1(
        ledger,
        project_id=project_id,
        epoch=request.epoch,
        command=_evidence_command(request.action),
        ...
        artifact_digest=_digest(b"target", request.target_commitment),
        policy_digest=_digest(b"reason", request.reason_digest),
        outcome="ALLOW" if enforced else "DENY",
    )
    return SentinelDefenseEvidenceResultV1(enforced=enforced, ledger=updated)
