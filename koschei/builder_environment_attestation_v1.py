...
def _payload(*, builder_id: str, environment_id: str, environment_digest: str,
             ...
             remote_evidence_digest: str, trust_root_id: str) -> bytes:
    rows = (
...
class BuilderEnvironmentAttestationV1:
    ...
    def assert_authenticated(self, *, environment_attestation_key: bytes,
        ...
        remote_evidence.assert_authenticated(
            remote_attestation_verifier_key=remote_attestation_verifier_key,
            raw_evidence_bytes=raw_remote_evidence_bytes,
        ...
        if remote_evidence.environment_digest != self.environment_digest:
            raise BuilderEnvironmentAttestationV1Error("remote attestation environment measurement mismatch")
        if remote_evidence.workload_digest != self.workload_digest:
            raise BuilderEnvironmentAttestationV1Error("remote attestation workload measurement mismatch")
        if self.remote_attestation_evidence_digest != remote_evidence.evidence_digest:
            raise BuilderEnvironmentAttestationV1Error("builder environment remote evidence mismatch")
        if self.trust_root_id != remote_evidence.trust_root_id:
            raise BuilderEnvironmentAttestationV1Error("builder environment trust-root mismatch")
        if self.epoch != remote_evidence.observed_epoch:
            raise BuilderEnvironmentAttestationV1Error("builder environment epoch differs from remote evidence")
        expected = hmac.new(key, _payload(
            ...
            remote_evidence_digest=self.remote_attestation_evidence_digest,
...
def attest_builder_environment_v1(*, builder_id: str, environment_id: str,
                                  ...
                                  remote_evidence: RemoteAttestationEvidenceV1,
                                  raw_remote_evidence_bytes: bytes,
                                  remote_attestation_verifier_key: bytes) -> BuilderEnvironmentAttestationV1:
    key = _key(environment_attestation_key)
    remote_evidence.assert_authenticated(
        remote_attestation_verifier_key=remote_attestation_verifier_key,
        raw_evidence_bytes=raw_remote_evidence_bytes,
        current_epoch=remote_evidence.observed_epoch,
    )
    ...
    if remote_evidence.environment_digest != environment_digest:
        raise BuilderEnvironmentAttestationV1Error("remote attestation environment measurement mismatch")
    if remote_evidence.workload_digest != workload_digest:
        raise BuilderEnvironmentAttestationV1Error("remote attestation workload measurement mismatch")
    result = BuilderEnvironmentAttestationV1(
        ...
        epoch=remote_evidence.observed_epoch,
        remote_attestation_evidence_digest=remote_evidence.evidence_digest,
        trust_root_id=remote_evidence.trust_root_id,
        attestation_digest="",
    )
    object.__setattr__(result, "attestation_digest", hmac.new(key, _payload(
        ...
        remote_evidence_digest=result.remote_attestation_evidence_digest,
        trust_root_id=result.trust_root_id,
    ), hashlib.sha256).hexdigest())
    result.assert_authenticated(
        environment_attestation_key=key,
        environment_bytes=environment_bytes, workload_bytes=workload_bytes,
        remote_evidence=remote_evidence,
        raw_remote_evidence_bytes=raw_remote_evidence_bytes,
        remote_attestation_verifier_key=remote_attestation_verifier_key,
        current_epoch=remote_evidence.observed_epoch,
    )
    return result
