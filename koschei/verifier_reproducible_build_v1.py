...
def _obs_payload(**v):
    rows=(f"builder={v['builder_id']}",f"verified_input={v['verified_input_digest']}",f"toolchain={v['toolchain_provenance_digest']}",f"environment={v['environment_attestation_digest']}",f"remote={v['remote_attestation_evidence_digest']}",f"profile={v['build_profile']}",f"artifact={v['artifact_digest']}","authority=0")
    return _OBS_CTX+"\n".join(rows).encode()

@dataclass(frozen=True,slots=True)
class VerifierBuilderObservationV1:
    builder_id:str; verified_input_digest:str; toolchain_provenance_digest:str; environment_attestation_digest:str; remote_attestation_evidence_digest:str; build_profile:str; artifact_digest:str; observation_digest:str; authority:bool=False; version:int=1
    def assert_authenticated(self, **k):
        ...
        k['environment'].assert_authenticated(environment_attestation_key=k['environment_attestation_key'],environment_bytes=k['environment_bytes'],workload_bytes=k['workload_bytes'],remote_evidence=k['remote_evidence'],raw_remote_evidence_bytes=k['raw_remote_evidence_bytes'],remote_attestation_verifier_key=k['remote_attestation_verifier_key'],current_epoch=k['current_epoch'])
...
class VerifierReproducibleBuildReceiptV1:
    ...
    def assert_authenticated(self, **k):
        ...
        a.assert_authenticated(builder_key=k['builder_a_key'],verified_input=k['verified_input'],mir=k['mir'],proof=k['proof'],toolchain=k['builder_a_toolchain'],toolchain_artifact_bytes=k['builder_a_toolchain_artifact_bytes'],toolchain_signing_key=k['builder_a_toolchain_signing_key'],environment=ea,environment_bytes=k['builder_a_environment_bytes'],workload_bytes=k['builder_a_workload_bytes'],environment_attestation_key=k['builder_a_environment_attestation_key'],remote_evidence=ra,raw_remote_evidence_bytes=k['builder_a_raw_remote_evidence_bytes'],remote_attestation_verifier_key=k['builder_a_remote_attestation_verifier_key'],current_epoch=k['current_epoch'],artifact_bytes=k['artifact_bytes'])
        b.assert_authenticated(builder_key=k['builder_b_key'],verified_input=k['verified_input'],mir=k['mir'],proof=k['proof'],toolchain=k['builder_b_toolchain'],toolchain_artifact_bytes=k['builder_b_toolchain_artifact_bytes'],toolchain_signing_key=k['builder_b_toolchain_signing_key'],environment=eb,environment_bytes=k['builder_b_environment_bytes'],workload_bytes=k['builder_b_workload_bytes'],environment_attestation_key=k['builder_b_environment_attestation_key'],remote_evidence=rb,raw_remote_evidence_bytes=k['builder_b_raw_remote_evidence_bytes'],remote_attestation_verifier_key=k['builder_b_remote_attestation_verifier_key'],current_epoch=k['current_epoch'],artifact_bytes=k['artifact_bytes'])
        ...
        digest=hmac.new(key,_repro_payload(verified_input_digest=self.verified_input_digest,artifact_digest=self.artifact_digest,builder_a_id=self.builder_a_id,builder_a_observation=self.builder_a_observation_digest,builder_a_toolchain=self.builder_a_toolchain_provenance_digest,builder_a_environment=self.builder_a_environment_attestation_digest,builder_a_remote=self.builder_a_remote_attestation_evidence_digest,builder_b_id=self.builder_b_id,builder_b_observation=self.builder_b_observation_digest,builder_b_toolchain=self.builder_b_toolchain_provenance_digest,builder_b_environment=self.builder_b_environment_attestation_digest,builder_b_remote=self.builder_b_remote_attestation_evidence_digest),hashlib.sha256).hexdigest()
...
def seal_reproducible_build_receipt_v1(**k):
    ...
    if ea.environment_id==eb.environment_id or ea.environment_digest==eb.environment_digest: raise VerifierReproducibleBuildV1Error('reproducible build requires distinct attested environments')
    if k.get('require_distinct_trust_roots',False) and ra.trust_root_id==rb.trust_root_id: raise VerifierReproducibleBuildV1Error('reproducible build policy requires distinct attestation trust roots')
    # full verification occurs before sealing
    for prefix,obs,env,remote in [('builder_a',a,ea,ra),('builder_b',b,eb,rb)]:
        obs.assert_authenticated(builder_key=k[prefix+'_key'],verified_input=k['verified_input'],mir=k['mir'],proof=k['proof'],toolchain=k[prefix+'_toolchain'],toolchain_artifact_bytes=k[prefix+'_toolchain_artifact_bytes'],toolchain_signing_key=k[prefix+'_toolchain_signing_key'],environment=env,environment_bytes=k[prefix+'_environment_bytes'],workload_bytes=k[prefix+'_workload_bytes'],environment_attestation_key=k[prefix+'_environment_attestation_key'],remote_evidence=remote,raw_remote_evidence_bytes=k[prefix+'_raw_remote_evidence_bytes'],remote_attestation_verifier_key=k[prefix+'_remote_attestation_verifier_key'],current_epoch=k['current_epoch'],artifact_bytes=k['artifact_bytes'])
    if a.artifact_digest!=b.artifact_digest: raise VerifierReproducibleBuildV1Error('independent builders produced different verifier artifacts')
    r=VerifierReproducibleBuildReceiptV1(k['verified_input'].build_input_digest,a.artifact_digest,a.builder_id,a.observation_digest,a.toolchain_provenance_digest,ea.attestation_digest,ra.evidence_digest,b.builder_id,b.observation_digest,b.toolchain_provenance_digest,eb.attestation_digest,rb.evidence_digest,'')
    key=_key(k['reproducibility_key'],'reproducibility_key'); object.__setattr__(r,'receipt_digest',hmac.new(key,_repro_payload(verified_input_digest=r.verified_input_digest,artifact_digest=r.artifact_digest,builder_a_id=r.builder_a_id,builder_a_observation=r.builder_a_observation_digest,builder_a_toolchain=r.builder_a_toolchain_provenance_digest,builder_a_environment=r.builder_a_environment_attestation_digest,builder_a_remote=r.builder_a_remote_attestation_evidence_digest,builder_b_id=r.builder_b_id,builder_b_observation=r.builder_b_observation_digest,builder_b_toolchain=r.builder_b_toolchain_provenance_digest,builder_b_environment=r.builder_b_environment_attestation_digest,builder_b_remote=r.builder_b_remote_attestation_evidence_digest),hashlib.sha256).hexdigest()); r.assert_authenticated(**k); return r
