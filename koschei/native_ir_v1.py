"""Koschei-native IR v1.

This IR exists to break the native value path away from the legacy fn/let/return
AST. It models only Koschei witness realities and their native domains.
"""

from __future__ import annotations

from dataclasses import dataclass

from .native_value_domains_v1 import (
    GLYPHS,
    TRUTH,
    WHOLE,
    MAX_GLYPHS_RESULT_BYTES,
    NativeValue,
    NativeValueGraph,
    dependency_order_value_graph,
)
from .semantic import INT_MAX, INT_MIN


class NativeIrError(ValueError):
    pass


@dataclass(frozen=True, slots=True)
class NativeIrAtomV1:
    literal: NativeValue | None = None
    witness: str | None = None


@dataclass(frozen=True, slots=True)
class NativeIrWitnessV1:
    name: str
    operation: str | None
    atoms: tuple[NativeIrAtomV1, ...]


@dataclass(frozen=True, slots=True)
class NativeIrRealityV1:
    witnesses: tuple[NativeIrWitnessV1, ...]
    resolve: str

    def by_name(self) -> dict[str, NativeIrWitnessV1]:
        return {item.name: item for item in self.witnesses}


def lower_value_graph_to_native_ir_v1(graph: NativeValueGraph) -> NativeIrRealityV1:
    """Lower native source semantics directly to Koschei-native IR.

    No legacy Program/FunctionDeclaration/LetStatement/ReturnStatement node is
    constructed on this path.
    """
    by_name = graph.by_name()
    lowered: list[NativeIrWitnessV1] = []
    for name in dependency_order_value_graph(graph):
        witness = by_name[name]
        atoms = tuple(
            NativeIrAtomV1(literal=atom.literal, witness=atom.witness)
            for atom in witness.term.atoms
        )
        lowered.append(NativeIrWitnessV1(name, witness.term.operation, atoms))
    return NativeIrRealityV1(tuple(lowered), graph.resolve)


def execute_native_ir_v1(ir: NativeIrRealityV1) -> NativeValue:
    """Execute native IR without reconstructing the legacy AST or interpreter."""
    values: dict[str, NativeValue] = {}

    def atom_value(atom: NativeIrAtomV1) -> NativeValue:
        if atom.literal is not None:
            return atom.literal
        if atom.witness is None or atom.witness not in values:
            raise NativeIrError("native IR references an unavailable witness")
        return values[atom.witness]

    for witness in ir.witnesses:
        if witness.name in values:
            raise NativeIrError("native IR contains duplicate witness identity")
        if not witness.atoms:
            raise NativeIrError("native IR witness has no atoms")
        if witness.operation is None:
            if len(witness.atoms) != 1:
                raise NativeIrError("native IR literal/reference witness has invalid arity")
            value = atom_value(witness.atoms[0])
        else:
            if len(witness.atoms) != 2:
                raise NativeIrError("native IR operation witness has invalid arity")
            left = atom_value(witness.atoms[0])
            right = atom_value(witness.atoms[1])
            op = witness.operation
            if op in {"sum", "difference", "product"}:
                if left.domain != WHOLE or right.domain != WHOLE:
                    raise NativeIrError(f"{op} requires whole/whole")
                a, b = int(left.value), int(right.value)
                result = a + b if op == "sum" else a - b if op == "difference" else a * b
                if not INT_MIN <= result <= INT_MAX:
                    raise NativeIrError(f"{op} exceeds signed Int64 reality")
                value = NativeValue(WHOLE, result)
            elif op == "same":
                if left.domain != right.domain:
                    raise NativeIrError("same requires identical native domains")
                value = NativeValue(TRUTH, left.value == right.value)
            elif op == "merge":
                if left.domain != GLYPHS or right.domain != GLYPHS:
                    raise NativeIrError("merge requires glyphs/glyphs")
                merged = str(left.value) + str(right.value)
                if len(merged.encode("utf-8")) > MAX_GLYPHS_RESULT_BYTES:
                    raise NativeIrError("merged glyphs exceeds native byte budget")
                value = NativeValue(GLYPHS, merged)
            else:
                raise NativeIrError(f"unsupported native IR operation {op!r}")
        values[witness.name] = value

    if ir.resolve not in values:
        raise NativeIrError("native IR resolve witness is unavailable")
    return values[ir.resolve]
