...
provider-specific parsing can execute.
"""
...
class RemoteAttestationEvidenceV1:
    ...
    def assert_authenticated(self,*,remote_attestation_verifier_key:bytes,raw_evidence_bytes:bytes,current_epoch:int)->None:
        ...
        observed=_epoch(self.observed_epoch); expires=_epoch(self.expires_before_epoch); generation=_generation(self.trust_anchor_generation)
...
def _seal(**v):
    key=_key(v['remote_attestation_verifier_key']); observed=_epoch(v['observed_epoch']); expires=_epoch(v['expires_before_epoch']); generation=_generation(v['trust_anchor_generation'])
    if expires<=observed: raise RemoteAttestationEvidenceV1Error("remote attestation expiry must be after observed epoch")
