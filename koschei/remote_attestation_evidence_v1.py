...
attestation-verifier artifact before provider-specific parsing can execute.
"""
from __future__ import annotations
from dataclasses import dataclass
import hashlib,hmac
from .attestation_verifier_abi_v1 import AttestationVerifierAbiV1,measure_attestation_verifier_artifact_v1
from .trust_anchor_admission_v1 import TrustAnchorManifestV1,TrustAnchorRuntimeAdmissionV1
...
class RemoteAttestationEvidenceV1:
    provider_id:str; trust_root_id:str; raw_evidence_digest:str; environment_digest:str; workload_digest:str; observed_epoch:int; expires_before_epoch:int; attestation_verifier_abi_digest:str; verifier_implementation_digest:str; trust_anchor_manifest_digest:str; trust_anchor_runtime_admission_digest:str; evidence_digest:str; verified:bool=True; authority:bool=False; version:int=1
...
def verify_remote_attestation_with_abi_v1(*,abi:AttestationVerifierAbiV1,verifier_artifact_bytes:bytes,raw_evidence_bytes:bytes,current_epoch:int,verifier,remote_attestation_verifier_key:bytes,trust_anchor_manifest:TrustAnchorManifestV1,trust_anchor_admission:TrustAnchorRuntimeAdmissionV1,root_signing_key:bytes,trust_anchor_runtime_admission_key:bytes)->RemoteAttestationEvidenceV1:
    current=_epoch(current_epoch); abi.assert_sealed()
    trust_anchor_admission.assert_authenticated(runtime_admission_key=trust_anchor_runtime_admission_key,root_signing_key=root_signing_key,manifest=trust_anchor_manifest,abi=abi,verifier_artifact_bytes=verifier_artifact_bytes,current_epoch=current)
    ...
    return _seal(provider_id=abi.provider_id,trust_root_id=outcome.trust_root_id,raw_evidence_bytes=raw_evidence_bytes,environment_digest=_measure_env(outcome.environment_bytes),workload_digest=_measure_workload(outcome.workload_bytes),observed_epoch=observed,expires_before_epoch=expires,verifier_abi_digest=abi.abi_digest,verifier_implementation_digest=abi.verifier_implementation_digest,trust_anchor_manifest_digest=trust_anchor_manifest.manifest_digest,trust_anchor_admission_digest=trust_anchor_admission.admission_digest,remote_attestation_verifier_key=remote_attestation_verifier_key)
