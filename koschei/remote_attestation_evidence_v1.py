...
Sanctioned issuance derives trust-root, measurements and freshness only from an
ABI-bound verifier callback over exact raw evidence bytes. Provider-specific TPM/TEE/
cloud parsing remains outside Lang core.
"""
...
class RemoteAttestationEvidenceV1:
    provider_id:str; trust_root_id:str; raw_evidence_digest:str; environment_digest:str; workload_digest:str; observed_epoch:int; expires_before_epoch:int; attestation_verifier_abi_digest:str; verifier_implementation_digest:str; evidence_digest:str; verified:bool=True; authority:bool=False; version:int=1
    def assert_authenticated(self,*,remote_attestation_verifier_key:bytes,raw_evidence_bytes:bytes,current_epoch:int)->None:
        ...
        observed=_epoch(self.observed_epoch); expires=_epoch(self.expires_before_epoch)
...
def _attest_remote_environment_evidence_v1(*,provider_id,trust_root_id,raw_evidence_bytes,environment_digest,workload_digest,observed_epoch,expires_before_epoch,verifier_abi_digest,verifier_implementation_digest,remote_attestation_verifier_key):
    key=_key(remote_attestation_verifier_key); observed=_epoch(observed_epoch); expires=_epoch(expires_before_epoch)
    if expires<=observed: raise RemoteAttestationEvidenceV1Error("remote attestation expiry must be after observed epoch")
    result=RemoteAttestationEvidenceV1(_text(provider_id,"provider_id"),_text(trust_root_id,"trust_root_id"),measure_raw_remote_attestation_v1(raw_evidence_bytes),_text(environment_digest,"environment_digest"),_text(workload_digest,"workload_digest"),observed,expires,_text(verifier_abi_digest,"verifier_abi_digest"),_text(verifier_implementation_digest,"verifier_implementation_digest"),"")
    object.__setattr__(result,"evidence_digest",hmac.new(key,_payload(provider_id=result.provider_id,trust_root_id=result.trust_root_id,raw_evidence_digest=result.raw_evidence_digest,environment_digest=result.environment_digest,workload_digest=result.workload_digest,observed_epoch=result.observed_epoch,expires_before_epoch=result.expires_before_epoch,verifier_abi_digest=result.attestation_verifier_abi_digest,verifier_implementation_digest=result.verifier_implementation_digest),hashlib.sha256).hexdigest())
    result.assert_authenticated(remote_attestation_verifier_key=key,raw_evidence_bytes=raw_evidence_bytes,current_epoch=observed); return result
def verify_remote_attestation_with_abi_v1(*,abi:AttestationVerifierAbiV1,verifier_artifact_bytes:bytes,raw_evidence_bytes:bytes,current_epoch:int,verifier,remote_attestation_verifier_key:bytes)->RemoteAttestationEvidenceV1:
    abi.assert_sealed(); current=_epoch(current_epoch)
    ...
    return _attest_remote_environment_evidence_v1(provider_id=abi.provider_id,trust_root_id=outcome.trust_root_id,raw_evidence_bytes=raw_evidence_bytes,environment_digest=_measure_env(outcome.environment_bytes),workload_digest=_measure_workload(outcome.workload_bytes),observed_epoch=observed,expires_before_epoch=expires,verifier_abi_digest=abi.abi_digest,verifier_implementation_digest=abi.verifier_implementation_digest,remote_attestation_verifier_key=remote_attestation_verifier_key)
