"""Sealed plan and artifact lineage for Koschei native-intelligence training v1.

Before compute starts, Koschei seals the exact base revision/weights, source
commit, constitutional holdout, balanced oracle corpus, exact exported JSONL byte
manifest, training configuration and method.

After compute, this module may seal adapter/checkpoint/log artifacts against that
plan. That artifact receipt is deliberately NOT sufficient to create a native
intelligence identity. The exact launch and run-start evidence must be joined by
``native_intelligence_training_execution_v1`` before model identity can exist.

No object in this module grants execution or deployment authority.
"""
...
class NativeTrainingPlanV1:
    ...
    def assert_sealed(self) -> None:
        ...
        if holdout in {corpus, export, train, validation, test}:
            raise NativeIntelligenceTrainingLineageError(
                "constitutional holdout cannot be reused as training material"
            )
...
class NativeTrainingReceiptV1:
    """Artifact receipt only; it is not a model identity receipt."""
...
def seal_native_training_plan_v1(
    ...
    *,
    ...
    """Seal exact balanced logical and byte-materialized training identity."""
...
def seal_native_training_receipt_v1(
    ...
    *,
    ...
    """Seal completed artifact evidence without granting model identity."""
...
def build_native_intelligence_from_training_receipt_v1(
    ...
    """Reject the obsolete artifact-receipt-to-model-identity shortcut."""

    plan.assert_sealed()
    receipt.assert_for(plan)
    raise NativeIntelligenceTrainingLineageError(
        "artifact receipt cannot create native intelligence; "
        "use an exact execution receipt"
    )
