...
The provider callback may run only after exact-artifact admission, reproducibility-gated
admission, a compact current-generation/lifetime/policy check, and exact local effect
binding. When both builder states are external-witness-confirmed, their witness receipt
digests are sealed into provider-native provenance.
"""
...
from .monotonic_witness_v1 import WitnessConfirmedGenerationStateV1
...
def _epoch(value:int)->int:
    if not isinstance(value,int) or isinstance(value,bool) or value<0: raise ProviderNativeVerifierV1Error("current_epoch must be a non-negative integer")
    return value
def _generation(value:int)->int:
    ...
    return value
def _policy(value:bool)->bool:
    if not isinstance(value,bool): raise ProviderNativeVerifierV1Error("required_distinct_trust_roots must be boolean")
    return value
...
def _payload(**v)->bytes:
    rows=(f"provider={v['provider_id']}",f"adapter_abi={v['adapter_abi_digest']}",f"verifier_implementation={v['verifier_implementation_digest']}",f"runtime_admission={v['runtime_admission_digest']}",f"reproducibility_receipt={v['reproducibility_receipt_digest']}",f"reproducible_runtime_admission={v['reproducible_runtime_admission_digest']}",f"builder_a_anchor={v['builder_a_anchor']}",f"builder_a_generation={v['builder_a_generation']}",f"builder_a_manifest={v['builder_a_manifest']}",f"builder_a_monotonic_witness={v.get('builder_a_monotonic_witness_receipt_digest','')}",f"builder_b_anchor={v['builder_b_anchor']}",f"builder_b_generation={v['builder_b_generation']}",f"builder_b_manifest={v['builder_b_manifest']}",f"builder_b_monotonic_witness={v.get('builder_b_monotonic_witness_receipt_digest','')}",f"effect_envelope={v['effect_envelope_digest']}",f"effect_receipt={v['effect_receipt_digest']}",f"effect_measurement={v['effect_measurement_digest']}",f"raw_response={v['raw_response_digest']}",f"expected_reference={v['expected_reference_digest']}",f"verified_reference={v['verified_reference_digest']}",f"provider_proof={v['provider_proof_digest']}",f"observed_epoch={v['observed_epoch']}",f"state={v['state']}","authority=0")
    return _CTX+"\n".join(rows).encode()
def _witness_digests(builder_a_generation_state,builder_b_generation_state)->tuple[str,str]:
    a=isinstance(builder_a_generation_state,WitnessConfirmedGenerationStateV1); b=isinstance(builder_b_generation_state,WitnessConfirmedGenerationStateV1)
    if a!=b: raise ProviderNativeVerifierV1Error("provider-native verification requires both builder generation states to use monotonic witnesses or neither")
    if not a: return "",""
    return builder_a_generation_state.witness_receipt_digest,builder_b_generation_state.witness_receipt_digest
def _assert_repro_admission_policy(reproducible_admission:VerifierReproducibleRuntimeAdmissionV1,reproducibility_receipt:VerifierReproducibleBuildReceiptV1)->None:
    ...
    if reproducible_admission.builder_a_trust_root_id!=reproducibility_receipt.builder_a_trust_root_id or reproducible_admission.builder_b_trust_root_id!=reproducibility_receipt.builder_b_trust_root_id or reproducible_admission.require_distinct_trust_roots is not reproducibility_receipt.require_distinct_trust_roots: raise ProviderNativeVerifierV1Error("provider-native reproducibility root policy differs from runtime admission")
def _assert_effect_binding(effect_envelope:EffectExecutionProofEnvelopeV1,effect_receipt:EffectExecutionReceiptV1,effect_result_bytes:bytes)->None:
    ...
    if effect_receipt.receipt_digest!=effect_envelope.effect_receipt_digest or effect_receipt.measurement_digest!=effect_envelope.measurement_digest or effect_receipt.operation!=effect_envelope.operation or effect_receipt.execution_epoch!=effect_envelope.epoch or effect_receipt.canonical_request_digest!=effect_envelope.canonical_request_digest: raise ProviderNativeVerifierV1Error("provider-native supplied effect receipt differs from effect envelope")
...
class ProviderNativeVerificationReceiptV1:
    ...
    def assert_authenticated(self,*,provider_native_verifier_key:bytes,adapter_abi:ProviderAdapterAbiV1,runtime_admission:VerifierRuntimeAdmissionV1,reproducible_admission:VerifierReproducibleRuntimeAdmissionV1,reproducibility_receipt:VerifierReproducibleBuildReceiptV1,reproducibility_key:bytes,builder_a_generation_state:TrustAnchorGenerationStateV1,builder_b_generation_state:TrustAnchorGenerationStateV1,provenance:VerifierBuildProvenanceV1,verifier_artifact_bytes:bytes,build_provenance_key:bytes,runtime_admission_key:bytes,reproducible_admission_key:bytes,effect_envelope:EffectExecutionProofEnvelopeV1,effect_receipt:EffectExecutionReceiptV1,effect_result_bytes:bytes,raw_response_bytes:bytes,current_epoch:int,required_distinct_trust_roots:bool=False)->None:
        ...
        reproducible_admission.assert_authenticated(reproducible_admission_key=reproducible_admission_key,base_admission=runtime_admission,adapter_abi=adapter_abi); _assert_repro_admission_policy(reproducible_admission,reproducibility_receipt)
        assert_reproducibility_current_v1(receipt=reproducibility_receipt,reproducibility_key=reproducibility_key,builder_a_generation_state=builder_a_generation_state,builder_b_generation_state=builder_b_generation_state,current_epoch=current,required_distinct_trust_roots=required)
        witness_a,witness_b=_witness_digests(builder_a_generation_state,builder_b_generation_state)
        if self.builder_a_monotonic_witness_receipt_digest!=witness_a or self.builder_b_monotonic_witness_receipt_digest!=witness_b: raise ProviderNativeVerifierV1Error("provider-native monotonic witness provenance mismatch")
...
def verify_provider_native_response_v1(*,provider_id:str,adapter_abi:ProviderAdapterAbiV1,runtime_admission:VerifierRuntimeAdmissionV1,reproducible_admission:VerifierReproducibleRuntimeAdmissionV1,reproducibility_receipt:VerifierReproducibleBuildReceiptV1,reproducibility_key:bytes,builder_a_generation_state:TrustAnchorGenerationStateV1,builder_b_generation_state:TrustAnchorGenerationStateV1,provenance:VerifierBuildProvenanceV1,verifier_artifact_bytes:bytes,build_provenance_key:bytes,runtime_admission_key:bytes,reproducible_admission_key:bytes,effect_envelope:EffectExecutionProofEnvelopeV1,effect_receipt:EffectExecutionReceiptV1,effect_result_bytes:bytes,raw_response_bytes:bytes,current_epoch:int,verifier:Callable[[bytes],ProviderNativeVerificationResultV1],provider_native_verifier_key:bytes,required_distinct_trust_roots:bool=False)->ProviderNativeVerificationReceiptV1:
    ...
    if current<effect_envelope.epoch: raise ProviderNativeVerifierV1Error("provider-native observation predates effect epoch")
